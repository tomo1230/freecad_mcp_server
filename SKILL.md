...
モデリングを始める前に必ず座標軸を確認する。

```
get_bounding_box または get_body_center で既存ボディの位置を把握する。
既存ボディがない場合は原点（0, 0, 0）を基準とする。
```

- X軸・Y軸・Z軸のどの方向が「高さ」「幅」「奥行き」に対応するかを把握してから形状を配置する
- FreeCAD では通常 Z軸が上方向
- 複数ボディを組み合わせる場合は、各ボディの基準点が揃っているかを確認してからブール演算に進む
...
### ルール1-b：フィレット前に必ず get_edges_info でエッジ型を確認する

add_fillet / add_chamfer は curve_type == "Circle" のエッジにのみ適用できる。
"Line" 型エッジを指定するとエラー「There are no suitable edges」が発生する。

必ずこの手順を踏む：
  1. get_edges_info(body_name="MyBody") でエッジ一覧を取得
  2. curve_type == "Circle" のインデックスだけを選ぶ
  3. add_fillet(edge_indices=[...) に渡す

  OK:  get_edges_info → Circle型のインデックスを抽出 → add_fillet
  NG:  インデックスを決め打ちで add_fillet を呼ぶ
...
補足：中空化に cut を使う場合も同様。
取っ手などの付属物は必ずソリッドの状態で join してから、内側 cut で中空化する。

  OK:  外形作成 → 取っ手join → 内側cut（中空化）
  NG:  外形作成 → 内側cut（中空化） → 取っ手join
...
| save_document | **ユーザーが「保存して」と明示的に指示した場合のみ呼ぶ。自動保存しない。** 保存時は必ずフルパスを渡す（例: `D:/freecad_mcp_server/MyPart.FCStd`）。パスなしで呼ぶと未保存ドキュメントでは失敗する |
...
```
【作業開始前】← 必ず最初に実行
  get_bounding_box / get_body_center  → 座標軸・既存ボディの位置を把握
  （既存ボディなし → 原点(0,0,0)を基準に設計開始）
